...
기출 연결: tic-tac-toe 게임 트리를 주고 **minimax 값을 노드마다 채워** 최적 수를 결정하는 유형. [26전기 기출]({{ site.baseurl }}/docs/notes/past-exams-26/) 참고.

핵심 깨달음 두 가지:

- minimax = **"상대가 최선을 둔다고 가정하고, 그 최악의 경우를 최대화"**. MAX는 자식 최대, MIN은 자식 최소를 말단에서 위로 전파.
- alpha-beta는 **결과를 바꾸지 않고** 자름. **"이미 확보한 더 나은 선택지가 있으면 이 가지는 더 볼 필요 없다"**(`α ≥ β`).

### Worked trace — minimax 값 채우기

2-ply 트리. 루트는 MAX, 그 아래 MIN 노드 3개(B1·B2·B3), 리프는 왼쪽부터 `[3,12,8] [2,4,6] [14,5,2]`.
...
→ 루트 값 `3`, 최적 수는 **왼쪽 가지 A1**.

### Worked trace — alpha-beta 가지치기

같은 트리를 왼쪽→오른쪽 DFS. `α`는 MAX 확보 최고값, `β`는 MIN 확보 최저값.

| step | 노드 | 관찰 | α / β | 동작 |
|---|---|---|---|---|
| 1 | B1 | 리프 3,12,8 전부 봄 | B1=3, 루트 α=3 | 끝까지 평가 |
| 2 | B2 | 첫 리프 `2` 봄 | B2의 β=2, α=3 | `β(2) ≤ α(3)` → 나머지 4,6 **prune** |
| 3 | B3 | 첫 리프 `14`, 다음 `5` | β=5, α=3 | 아직 `β > α`, 계속 |
| 3 | B3 | 셋째 리프 `2` 봄 | β=2, α=3 | `β(2) ≤ α(3)` → (남은 게 있었다면) prune |

핵심: B2는 첫 자식 `2`만 보고 "MIN이라 2 이하로 떨어질 텐데, MAX는 이미 3을 확보" → 나머지 안 봄. 결과(루트=3)는 minimax와 동일.

### 보강

- 가지치기 효과는 **move ordering**에 좌우됨. 좋은 순서(각 노드의 최선 수가 가장 왼쪽)면 `O(b^(d/2))`로 줄어 같은 시간에 약 **2배 깊이** 탐색 가능.
